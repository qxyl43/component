$(function(){
    ...
    var  book_data=[
        ...
        ,{ // 两条两条 当type类型是1时是右边一页
            ...
            sortNum: 8,
            title: "111您希望哪些日常工作或业务，可以使用技术替代当前手工操作？",
            img: "images/page_L5.jpg",
            num: 2,
            text: [],
            inputText:'',
    ...
    function creat_html(data,num){
        ...
        for(var i=0; i<data.length; i++){
            if(b_ok){
                ...
                if(type == 2){  // 左边页 注意type=2
                    ...
                    var preHtml='';
                    if (i !== 0 ) preHtml='<p class="pre_page">上一题</p>'
                    ...
                    if (data.length%2 ==0){
                      ...
                      '</div>'
                       ...
                       page_html += '<div class="page_L1 cover">'+
                        ...
                        '<div class="page-num"><u>'+data[i].sortNum+'</u>/'+data.length+'</div>'+
                        '<div class="book-block"><p class="w_title">'+title+'</p>' + selectHtml +'</div>' +
                        '<p class="pre_page">上一题</p><p class="next_page" >完成</p> ' +
                        '</div>'
                    }
                }
                html+=page_html;
        ...
        if(b_ok){
            ...
            $('.flipbook').turn({
                ...
                page:1, // 指定在第几页
                gradients: true,
                autoCenter: true, // direction 这个功能不能用
                display:'double', // single单页　在这里不能用的
                when: { 
                    turning: function(event, page, pageObject) {
                      ...
                      if (page == 1) {
                         ...
                         //$(this).turn('peel','br'); 直接折角，没有反映
                    } 
                }
            });
            
           $('.book-close').on('click',  function(event) {
             $('.book-mask').addClass('on-mask')
           }); 
           $('.book-open').on('click',  function(event) {
             $('.book-mask').removeClass('on-mask')
           }); 
